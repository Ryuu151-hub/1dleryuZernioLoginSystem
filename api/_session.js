...
export function setSession(res, data) {
  res.setHeader("Set-Cookie", cookie(COOKIE, pack({...data, exp: Date.now()+30*24*60*60*1000}), 30*24*60*60));
}
...
export function setState(res, data) {
  res.setHeader("Set-Cookie", cookie(STATE_COOKIE, pack({...data, exp: Date.now()+10*60*1000}), 600));
}
