...
Use `https://1dleryu-zernio-login-system.vercel.app` as the backend URL in the frontend and set `APP_URL` to the same URL. The Zernio launch uses a normal browser redirect to `/api/auth/start`, so the initial OAuth launch does not depend on cross-origin `fetch()`.
